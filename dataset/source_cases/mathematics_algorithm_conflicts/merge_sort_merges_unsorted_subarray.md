...
**拓展方式**：换数字。
**初始状态、动作与冲突终态**：左子列 `[6,2]`、右子列 `[4,8]`；在归并排序中跳过左侧内部排序，直接逐次比较两侧头元素，输出 `[4,6,2,8]`。
**正常规则及结果**：应先将左侧排成 `[2,6]`，再归并得到 `[2,4,6,8]`。

##### 新增变体 M10

**拓展方式**：原始总序列增加两个元素。
**初始状态、动作与冲突终态**：左子列 `[5,1,7]`、右子列 `[2,4,8]`；跳过左侧排序，直接按头元素归并，依次输出 `2、4、5、1、7、8`。
**正常规则及结果**：左侧先排成 `[1,5,7]`，最终应为 `[1,2,4,5,7,8]`。
